...
fn main() -> Result<(), anyhow::Error> {
    ...
    match args.command {
        Some(Commands::Run { file }) => {
            ...
            let program: Program<NamedDeBruijn> = Program::<FakeNamedDeBruijn>::from_flat(&bytes)?.into();
            ...
            let mut states = vec![(state.clone(), machine.ex_budget.clone())];
            loop {
                state = machine.step(state).unwrap();
                states.push((state.clone(), machine.ex_budget.clone()));
                if let MachineState::Done(t) = state {
                    ...
                    println!("Budget -- Memory: {} ({:.3}%)   CPU: {} ({:.3}%)",
                        spent_mem, 100. * (spent_mem as f64) / 14000000.,
                        spent_cpu, 100. * (spent_cpu as f64) / 10000000000.
                    );
            ...
            loop {
                ...
                println!("Cursor: {:5} -- Next Instruction: {} -- Budget Spent: {} mem  {} cpu", cursor, state, spent_mem, spent_cpu);
                ...
                match &*buffer {
                    ...
                    "c\n" => {
                        match &states[cursor].0 {
                            MachineState::Return(c, _) |
                            MachineState::Compute(c, _, _) => println!("Context: {:?}", c),
                    ...
                    "e\n" => {
                        match &states[cursor].0 {
                            ...
                            MachineState::Return(_, _) => println!("Machine is returning a value, no Env."),
                            MachineState::Done(_) => println!("Machine is done, no context."),
                        }
                    }
                    "v\n" => {
